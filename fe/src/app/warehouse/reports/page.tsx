...
import { Search, X, Eye, ChevronLeft, ChevronRight, Upload, CheckCircle } from 'lucide-react'
...
function DetailModal({
  ...
  report: DamageReportView
  onClose: () => void
  onApproved?: (updatedReport: DamageReportView) => void
  ...
  const handleApprove = async () => {
    try {
      ...
      const approvedData = await DamageReportAPIService.approveDamageReport(report.id)
      if (approvedData) {
        // Update the current report with new approval status
        const updatedReport: DamageReportView = {
          ...report,
      ...
      setApproveError(error?.message || 'Không thể duyệt báo cáo thiệt hại.')
    } finally {
      setApproving(false)
    }
  }
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              { label: 'Ngày báo cáo', value: formatDateTime(report.reportedDate) },
          ...
          {approveError && (
            <div className="px-4 py-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-700">{approveError}</div>
          )}
          ...
          {report.status === 'PENDING' && (
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                disabled={approving}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white rounded-xl text-sm font-semibold transition-colors"
              >
                <CheckCircle size={16} />
                {approving ? 'Đang duyệt...' : 'Duyệt báo cáo'}
              </button>
            </div>
          )}
        </div>
...
function CreateModal({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!locationId) {
      setSubmitError('Không tìm thấy workplace_id của người dùng hiện tại.')
      return
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500">Gửi báo cáo với đầy đủ thông tin</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-slate-100 text-slate-500">
            <X size={18} />
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="VD: hư hại, vỡ, ẩm mốc"
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                required
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Số lượng sản phẩm bị hư hại"
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                required
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">ID Địa điểm (workplace_id)</label>
              <input
                type="text"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">Mô tả chi tiết <span className="text-red-500">*</span></label>
            <textarea
              rows={3}
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Mô tả đầy đủ về nguyên nhân, vị trí, và mức độ thiệt hại"
...
export default function DamageReportsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const [productRows, reportRows, usersList] = await Promise.all([
          ...
          locationId
            ...
            : Promise.resolve([]),
          UserAPIService.getIamUsersList(),
        ])
        ...
        if (usersList && usersList.length > 0) {
          usersList.forEach((user: UserInfoFromAPI) => {
            const userId = String(user.id || '').trim()
            const fullName = (user.fullName || user.full_name || user.name || '').trim()
            
            if (userId && fullName) {
              idToNameMap.set(userId, fullName)
            }
          })
        }
        
        setUserNameMap(idToNameMap)
      } catch (error: any) {
  ...
  const handleCreate = async (payload: CreatePayload) => {
    if (!locationId) {
      throw new Error('Không tìm thấy workplace_id của người dùng hiện tại.')
    }
  ...
  const handleApprovedReport = (updatedReport: DamageReportView) => {
    setReports((prev) =>
    ...
    <div className="p-6 bg-slate-50 min-h-screen">
      {selectedReport && (
        <DetailModal
          ...
          onApproved={handleApprovedReport}
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <p className="text-xs text-slate-400 mb-1">Báo cáo &gt; Lịch sử báo cáo thiệt hại</p>
      ...
      {!locationId && (
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Người dùng hiện tại chưa có workplace_id nên không thể gửi báo cáo thiệt hại.
        </div>
      )}
            ...
            {paginated.map((report) => (
              <tr key={report.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-4 text-right">
                  <span className="font-semibold text-slate-800 text-xs whitespace-nowrap">{report.quality}</span>
                </td>
                <td className="px-4 py-4">
                  <StatusBadge status={report.status} />
                </td>
                <td className="px-4 py-4 text-xs text-slate-500 whitespace-nowrap">{formatDateTime(report.reportedDate)}</td>
